...
def intervals_lt_x(L: List[Num], x: Num) -> Tuple[int, int, List[Tuple[int]]]:
    """
    正の数列 L の幅 1 以上の全区間について、区間和が x 以下であるようなものを O(n) で探索する。
    ...
    Examples:
        ...
        (10, 3, [(1, 4)])
        >>> intervals_lt_x([4, 6, 7, 8, 1, 2, 110, 2, 4, 12, 3, 9], 25)
        (32, 5, [(1, 6)])
    """
    assert all(map(lambda elm: elm>0, L))
    assert len(L) > 0
    assert x > 0
    n = len(L)
...
def intervals_gt_x(L: List[Num], x: Num) -> Tuple[int, int, List[Tuple[int]]]:
    """
    正の数列 L の幅 1 以上の全区間について、区間和が x 以上であるようなものを O(n) で探索する。
    ...
    Examples:
        ...
        (8, 2, [(1, 3), (3, 5)])
        >>> intervals_gt_x([4, 6, 7, 8, 1, 2, 110, 2, 4, 20, 3, 9], 25)
        (51, 1, [(6, 7)])
    """
    assert all(map(lambda elm: elm>0, L))
    assert len(L) > 0
    ...
    for left in range(n):
        ...
        # これは summation < x で判定可能。(right_start が末尾かどうかで判定してはダメ。summation < x => 末尾は真だけれど...)
        while right_start < n and summation < x:
            summation += L[right_start]
            right_start += 1

        # これ以上 left を進めても満たす区間は存在しない
        if summation < x:
            break
        # 尺取りで完全に区間が潰れたら right_start も動かす
        if right_start == left:
            continue
