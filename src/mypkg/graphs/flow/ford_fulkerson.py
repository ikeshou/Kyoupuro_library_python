...
from typing import Sequence, List, Union, Any, Optional
...
class Edge:
    def __init__(self, here: int, to: int, capacity: Num, is_rev: bool, rev_edge: Optional['Edge']=None):
        self.here = here
        self.to = to
...
class FordFulkerson:
    ...
    def _bfs(self, start_ind: int, goal_ind: int) -> List[Edge]:
        """
        capacity > 0 の辺のみ連結し
        ていると見なした有向グラフについて、start_ind ~ goal_ind の経路を BFS で一つ発見して返す
        頂点を enqueue する代わりに、Edge を enqueue している。
        """
        prev_edge_memo = [None] * self.num_of_v
        visited = [False] * self.num_of_v
        q = deque()
        q.append(Edge(None, start_ind, None, is_rev=False, rev_edge=None))    # 擬似エッジ
        while q:
            previous_e = q.popleft()
            u = previous_e.to    # 現在地
            if not visited[u]:
                visited[u] = True
                prev_edge_memo[u] = previous_e
                if u == goal_ind:
                    # 経路復元
                    path = []
                    while u is not None:    # 擬似エッジを踏むまで
                        path.append(prev_edge_memo[u])
                        u = prev_edge_memo[u].here
                    path.reverse()
                    return path[1:]    # 最初の擬似エッジを取り除く
                for e in self.graph[u]:
                    if not visited[e.to] and e.capacity > 0:
