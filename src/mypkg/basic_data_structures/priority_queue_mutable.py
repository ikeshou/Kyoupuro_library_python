...
peek():
    O(1)
    優先度が (最小: min-pqueue / 最大: max-pqueue) の task オブジェクトを確認し、(task, priority) を返す (ヒープからは取り出さない)
...
class OriginalPQueue:
    ...
    def peek(self) -> Any:
        """O(1) でヒープトップのエントリを盗み見て、その (タスク, 優先度) を返す"""
        if self.empty():
    ...
    def pop_task(self) -> Any:
        """O(lgn) でヒープトップのエントリをポップし、その (タスク, 優先度) を返す"""
        if self.empty():
            raise KeyError(f"OriginalPQueue.pop_task(): pqueue is empty.")
        priority, _, task = self._heappop()
        return (task, priority)
