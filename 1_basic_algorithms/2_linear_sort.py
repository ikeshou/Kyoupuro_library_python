...
if __name__ == "__main__":
    ...
    for i in range(300):
        ...
        buf = [random.randint(0, 1000) for _ in range(size)]
        sorted_buf = sorted(buf)
        assert(counting_sort(buf, 1000) == sorted_buf)
        counting_sort_destructive(buf, 1000)
        assert(buf == sorted_buf)
    print("- test for counting_sort() / counting_sort_destructive() passed.")
    
    # test for radix sort
    for i in range(300):
        size = random.randint(0, 1000)
        bucket_bit_size = random.randint(1, 16)
        buf = [random.randint(0, 1000) for _ in range(size)]
        sorted_buf = sorted(buf)
        radix_sort(buf, bucket_bit_size)
        assert(buf == sorted_buf)
    print("- test for radix_sort() passed.")
